 ...
 */
static void afficherQuadtreeRec(Quadtree qt, int W, int wmin) {
 ...
 */
static void MLV_draw_text_box_centered(int x, int y, int width, int height, const char* text, int size_interligne, MLV_Color border_color, MLV_Color text_color, MLV_Color background_color, MLV_Text_justification text_justification, MLV_Horizontal_position horizontal_position, MLV_Vertical_position vertical_position) {
...
void afficherMenu(int choix, int X, int Y) {
    ...
    MLV_draw_text_box_centered(center[0], center[1] - 200, 400, 50, "Quadtree", 0, MLV_COLOR_WHITE, MLV_COLOR_WHITE, MLV_COLOR_GREY30, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
    MLV_draw_text_box_centered(center[0], center[1] - 50, 200, 50, text[choix], 0, MLV_COLOR_WHITE, MLV_COLOR_WHITE, MLV_COLOR_GREY30, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
    MLV_draw_text_box_centered(center[0], center[1] + 50, 200, 50, "Lancer", 0, MLV_COLOR_WHITE, MLV_COLOR_WHITE, MLV_COLOR_GREY30, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
    MLV_draw_text_box_centered(center[0], center[1] + 150, 200, 50, "Quitter", 0, MLV_COLOR_WHITE, MLV_COLOR_WHITE, MLV_COLOR_GREY30, MLV_TEXT_CENTER, MLV_HORIZONTAL_CENTER, MLV_VERTICAL_CENTER);
}
...
int clicMenu(int x, int y) {
    ...
    if (x > center[0] - 100 && x < center[0] + 100) {
        if (y > center[1] - 75 && y < center[1] - 25) {
            return 2;
        }
        if (y > center[1] + 25 && y < center[1] + 75) {
            return 1;
        }
        if (y > center[1] + 125 && y < center[1] + 175) {
            return 3;
        }
    }
    return 0;
